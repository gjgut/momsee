...
import com.example.android0211.Refactor.Util.PolicyManager;
        ...
        compositeDisposable.add(myAPI.loginUser_child(email, child_name)
                ...
                .subscribe(s -> {
                    if (s.contains("child_name")) {
                        Toast.makeText(childName.this, "로그인 성공", Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent(childName.this, childMain.class);
                         intent.putExtra("email",email);
                         intent.putExtra("child_name",child_name);

                        startActivity(intent);
                    } else
                        Toast.makeText(childName.this, "" + s, Toast.LENGTH_SHORT).show();
                }));
